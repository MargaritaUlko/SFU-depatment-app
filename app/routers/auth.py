...
from app.crud.user import get_user_by_email, create_user, authenticate_user, get_user
from app.models.user import Role
from app.schemas.user import (
    UserCreate, UserRead, LoginRequest, TokenResponse, RefreshRequest, LogoutRequest,
)
...
async def register(data: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    # Роль всегда student при самостоятельной регистрации.
    # Повышение роли — только через PATCH /users/{id}/role от admin.
...
async def refresh_tokens(data: RefreshRequest, db: AsyncSession = Depends(get_db)):
    ...
    # Ротация токена: отзываем старый, выдаём новый
